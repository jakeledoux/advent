...
fn dijkstra(map: CaveMap) -> Option<Danger> {
    let (start_pos, end_pos) = ((0, 0), (map.width - 1, map.height - 1));

    let mut min_danger: HashMap<Pos, Danger> = HashMap::new();
    let mut best_parent: HashMap<Pos, Pos> = HashMap::new();
    let mut queue: Vec<Pos> = vec![start_pos];

    for (pos, _) in map.cells() {
        min_danger.insert(pos, if pos == start_pos { 0 } else { Danger::MAX });
    }
    ...
    while !queue.is_empty() {
        ...
        // Found target
        if pos == end_pos {
            return Some(min_danger[&end_pos]);
        }

        for (neighbor_pos, neighbor_danger) in map.get_adjacent_cells(pos) {
            let alt = min_danger[&pos] + neighbor_danger;
            if alt < min_danger[&neighbor_pos] {
                min_danger.insert(neighbor_pos, alt);
                best_parent.insert(neighbor_pos, pos);

                let idx = queue
                    .binary_search_by(|other| alt.cmp(&min_danger[other]))
                    .unwrap_or_else(|i| i);
...
fn parse_input(input: &'static str, upscale: usize) -> CaveMap {
